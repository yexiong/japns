...
public class ApnsConnectionResendTest {
	...
	private ApnsService build(ApnsDelegate delegate) {
		return APNS.newService().withSSLContext(clientContext()).withGatewayDestination(LOCALHOST, apnsSim.getEffectiveGatewayPort()).withFeedbackDestination(LOCALHOST, apnsSim.getEffectiveFeedbackPort()).withDelegate(delegate).build();
	}
